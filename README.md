...
**首选：按页范围无损拆分**（画质和原文件完全一致）

```bash
npm run pdf:split -- "_原始文件/原版.pdf" "docs/public/资料/大一春夏/常微分方程" "ode笔记" 1-15 16-37 38-52
```

页码 1 起、含两端，自动补零对齐。矢量内容原样搬运，**零失真**，但每次只看一部分，加载就快了。实测：

| 分块 | 页数 | 大小 |
| --- | --- | --- |
| `ode笔记-P01-15.pdf` | 15 | 6.44 MB |
| `ode笔记-P16-37.pdf` | 22 | 10.11 MB |
| `ode笔记-P38-52.pdf` | 15 | 5.40 MB |

> 想再快就把范围切细一点，比如 `1-15 16-26 27-37 38-52`，把最大那份压到 5 MB 左右。切得越细单次加载越快，代价是文件数变多。

**备选：栅格化压缩**（体积能到 1/10，但画质有损）

```bash
npm run pdf:shrink -- "docs/public/资料/xxx/笔记.pdf" --dpi=300 --quality=85
```

| 参数 | 默认 | 说明 |
| --- | --- | --- |
| `--dpi=N` | 150 | 渲染精度。**这个是画质关键**，细笔画发虚就是它不够 |
| `--quality=N` | 70 | JPEG 质量。笔画周围出现块状噪点就是它太低 |
| `--gray` | 关 | 转灰度，再省 15%～20%（有彩色笔迹别开） |
| `--pages=a-b` | 全部 | 只处理部分页，用来试参数 |
| `--jpg=N:文件` | — | 只把第 N 页导成 JPG，方便肉眼对比画质 |

| 档位 | 大小 | 压缩比 |
| --- | --- | --- |
| 150 DPI / q70 | 1.25 MB | 17.5x |
| 200 DPI / q75 | 2.00 MB | 11x |
| 300 DPI / q85 | 3.92 MB | 5.6x |
| 400 DPI / q85 | 6.41 MB | 3.4x |

**栅格化的两个固有代价**：① 有分辨率上限，放大超过某个倍数就发虚；② JPEG 会留压缩痕迹。**要完美画质就别用栅格化，用上面的拆分。**
