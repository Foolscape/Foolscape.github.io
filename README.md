...
手写笔记用触控笔导出时，每一笔都是**矢量路径**、坐标还精确到小数点后 6 位 —— 一页能有 7 万条绘图指令，体积会大得离谱（实测一份 52 页的笔记导出成 21.96 MB）。用这条命令栅格化瘦身：

```bash
npm run pdf:shrink -- "docs/public/资料/大一春夏/常微分方程/ode笔记.pdf"
```

| 参数 | 默认 | 说明 |
| --- | --- | --- |
| `--dpi=N` | 150 | 渲染精度。手写笔记 150 够用，要放大看细节用 200 |
| `--quality=N` | 70 | JPEG 质量 1–100 |
| `--gray` | 关 | 转灰度。纯黑白笔记可开，再省 15%～20%（有彩色笔迹别开） |
...
| 300 DPI / q80 | 3.92 MB | 5.6x |

**代价**：变成图片后文字不可选中、不可搜索，放到很大时会略微发虚。**原文件不会被修改**，输出默认叫 `xxx-压缩.pdf`，你翻一遍确认清晰度后再替换。

> 大体积的原始文件放 `_原始文件/`（已在 `.gitignore` 里），只留本地，不提交、不部署。
